...
| Pending, ambiguous, low-confidence, unsupported event | Keep the current video unchanged. |
...
Until a checkpoint is calibrated, the detector may identify it and the API will report a candidate, but it will not issue a playback revision. This is deliberate: showing the correct game at the wrong moment would make the demo less credible.
