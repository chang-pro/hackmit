...
The product loop: point a camera at a sports broadcast, identify one of four indexed events and its current checkpoint, replace the camera view with the synchronized predownloaded broadcast, then estimate and compare its win probability without a sport picker.

> **Current live path.** `/phone` or the native glasses companion supplies the detector view; only user-gated snapshots reach `LiveEventAnalyzer`, where Cerebras Gemma extracts a universal event observation from the earliest and latest representatives of each five-frame temporal window. `services/demo/intelligence.js` exact-matches one of four historical packs. At ≥70% event confidence, `PlaybackDirector` emits a revision immediately: exact checkpoints use calibrated offsets, while in-between states interpolate media time from phase/clock anchors without unlocking prediction values. `/capture` double-buffers the matching local MP4, seeks once, and leaves it playing while later same-event insights update prediction, mock market, evidence, and research. The older `pipeline.js` fixture path remains available through `/api/comparison` for deterministic lower-level tests.
