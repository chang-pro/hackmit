...
test("Gemini Flash Lite classifies one WebRTC snapshot and selects the matching replay", async () => {
  ...
  try {
    const backend = createGeminiBackend({
      fetchImpl: async (url, options) => {
        ...
        return {
          ok: true,
          json: async () => ({
            candidates: [{ content: { parts: [{ text: '{"classification":"mma","confidence":0.94}' }] } }],
          }),
        };
      },
    });
    ...
    assert.deepEqual(request.body.generationConfig.responseSchema.properties.classification.enum, [
