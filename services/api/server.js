...
const DEFAULT_FIXTURE = "frame_000184";
  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (req.method === "OPTIONS") {
        ...
        if (requestedFixture && !KNOWN_FIXTURES.has(requestedFixture)) {
          sendJson(res, 400, {
          ...
          sendJson(
            ...
            200,
            await runPipeline(
              join(FIXTURES_DIR, `${requestedFixture}.json`),
              undefined,
              null,
              { reconciler: new Reconciler() }
            )
          );
        } else if (latestInsight && Date.now() - latestInsightAt <= LIVE_SESSION_TTL_MS) {
          sendJson(res, 200, latestInsight);
        } else {
          sendJson(
            res,
            200,
            await runPipeline(join(FIXTURES_DIR, `${DEFAULT_FIXTURE}.json`))
          );
        }
      } else if (
        req.method === "GET" &&
