...
The camera/glasses feed is now the selector, not the permanent theater video. Cerebras detects which of four canonical broadcasts is visible and resolves one of 27 deterministic checkpoints. At 70% or higher event-match confidence, a server-owned `PlaybackDirector` immediately swaps to the matching predownloaded MP4. An exact checkpoint uses its calibrated offset; an in-between state interpolates a seek from the visible phase and clock while probabilities remain gated. Later model results from the same game update predictions but never restart or reseek the video. Only a newly detected `pack_id` creates another revision and switches the theater.
